...
interface GlowCardProps {
  ...
  /**
   * When true the component renders as a plain block container (no fixed
   * aspect-ratio / grid rows) — ideal for content-driven card sizes.
   */
  customSize?: boolean
}
...
// Injected once per page to avoid duplicating the <style> block
...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const layoutClasses = customSize
    ? 'block'
    : 'grid grid-rows-[1fr_auto] aspect-[3/4]'
  ...
  return (
    <div
      ...
      data-glow
      style={inlineStyles}
      className={`${sizeClasses} ${layoutClasses} rounded-2xl relative shadow-[0_1rem_2rem_-1rem_black] backdrop-blur-[5px] ${className}`}
    >
